...
# Uses C_PiperInterface_V2 (SDK v2 protocol).
import rclpy
...
class PiperRosNodeV2(Node):
    """ROS2 node for the robotic arm (SDK v2)"""

    def __init__(self) -> None:
        super().__init__('piper_ctrl_single_node_v2')
        # ROS parameters
        self.declare_parameter('can_port', 'can0')
        self.declare_parameter('auto_enable', False)
        self.declare_parameter('gripper_exist', True)
        self.declare_parameter('gripper_val_mutiple', 1)
        # Per-joint MIT gains (kp reference: 10.0, kd reference: 0.8)
        for i in range(1, 7):
            self.declare_parameter(f'joint{i}_kp', 10.0)
            self.declare_parameter(f'joint{i}_kd', 0.8)
        ...
        self.mit_kp = [self.get_parameter(f'joint{i}_kp').get_parameter_value().double_value for i in range(1, 7)]
        self.mit_kd = [self.get_parameter(f'joint{i}_kd').get_parameter_value().double_value for i in range(1, 7)]
        ...
        self.get_logger().info(f"MIT kp: {self.mit_kp}")
        self.get_logger().info(f"MIT kd: {self.mit_kd}")
        # Publishers
        ...
        # Create piper v2 interface and open CAN port
    ...
    def PublishArmJointAndGripper(self):
        ...
        # Raw data is in degrees * 1000; convert to radians
    ...
    def joint_callback(self, joint_data):
        """Callback function for joint angles (MIT control mode)"""
        # Build name->index map for flexible joint ordering
        joint_positions = {}
        joint_velocities = {}
        joint_6 = 0

        for idx, joint_name in enumerate(joint_data.name):
            joint_positions[joint_name] = joint_data.position[idx]  # radians
            if idx < len(joint_data.velocity):
                joint_velocities[joint_name] = joint_data.velocity[idx]  # rad/s
        ...
        if self.GetEnableFlag():
            # MIT mode: MOVE M (0x04) with is_mit_mode=0xAD
            self.piper.MotionCtrl_2(0x01, 0x04, 0, 0xAD)

            joint_names = ['joint1', 'joint2', 'joint3', 'joint4', 'joint5', 'joint6']

            for motor_num, joint_name in enumerate(joint_names, start=1):
                pos_rad = joint_positions.get(joint_name, 0.0)
                vel_rad = joint_velocities.get(joint_name, 0.0)
                kp = self.mit_kp[motor_num - 1]
                kd = self.mit_kd[motor_num - 1]
                self.piper.JointMitCtrl(motor_num, pos_rad, vel_rad, kp, kd, 0.0)
...
def main(args=None):
    ...
    piper_single_node_v2 = PiperRosNodeV2()
    try:
        rclpy.spin(piper_single_node_v2)
    except KeyboardInterrupt:
        pass
    finally:
        piper_single_node_v2.destroy_node()
        rclpy.shutdown()
